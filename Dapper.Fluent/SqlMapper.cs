/*
 * Got from https://github.com/SamSaffron/dapper-dot-net/blob/master/Dapper/SqlMapper.cs.
 */
/*
...
namespace Dapper
{
    ...
    static partial class SqlMapper
    {
        ...
        partial class Link<TKey, TValue> where TKey : class
        {
            ...
            public static bool TryAdd(ref Link<TKey, TValue> head, TKey key, ref TValue value)
            {
                ...
                do
                {
                    ...
                    { // existing match; report the existing param instead
                        value = found;
                        return false;
                    }
        ...
        internal static DbType LookupDbType(Type type, string name)
        {
            ...
            throw new NotSupportedException(string.Format("The member {0} of type {1} cannot be used as a parameter param", name, type));
        }
...
this IDbConnection cnn, string sql, Func<TFirst, TSecond, TThird, TFourth, TReturn> map, dynamic param = null, IDbTransaction transaction = null, bool buffered = true, string splitOn = "Id", int? commandTimeout = null, CommandType? commandType = null
            ...
            , IDictionary<string, object>
        {
            ...
            public bool TryGetValue(string name, out object value)
            {
                ...
                // exists, **even if** we don't have a param; consider table rows heterogeneous
        ...
        {
            if (value == null || value is DBNull) throw new ArgumentNullException("param");
            string s = value as string;
            if (s == null || s.Length != 1) throw new ArgumentException("A single-character was expected", "param");
            return s[0];
        }
        ...
        public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity, bool checkForDuplicates)
        {
            ...
            foreach (var prop in props)
            {
                ...
                if (dbType == DynamicParameters.EnumerableMultiParameter)
                {
                    ...
                    il.Emit(OpCodes.Callvirt, prop.GetGetMethod()); // stack is [parameters] [command] [name] [typed-param]
                    if (prop.PropertyType.IsValueType)
                    {
                        il.Emit(OpCodes.Box, prop.PropertyType); // stack is [parameters] [command] [name] [boxed-param]
                    }
                ...
                il.Emit(OpCodes.Callvirt, prop.GetGetMethod()); // stack is [parameters] [[parameters]] [parameter] [parameter] [typed-param]
                bool checkForNull = true;
                if (prop.PropertyType.IsValueType)
                {
                    il.Emit(OpCodes.Box, prop.PropertyType); // stack is [parameters] [[parameters]] [parameter] [parameter] [boxed-param]
                    if (Nullable.GetUnderlyingType(prop.PropertyType) == null)
                    {   // struct but not Nullable<T>; boxed param cannot be null
                        checkForNull = false;
                    }
                }
                if (checkForNull)
                {
                    ...
                    // relative stack: [boxed param]
                    il.Emit(OpCodes.Dup);// relative stack: [boxed param] [boxed param]
                    Label notNull = il.DefineLabel();
                    Label? allDone = dbType == DbType.String ? il.DefineLabel() : (Label?)null;
                    il.Emit(OpCodes.Brtrue_S, notNull);
                    // relative stack [boxed param = null]
                    ...
                    // relative stack [boxed param or DBNull]
                }
...
#pragma warning restore 618
            ...
            {   // assume the param is returned as the correct type (int/byte/etc), but box back to the typed enum
                return r =>
                {
...
)
        {
            ...
            foreach (var item in members)
            {
                if (item != null)
                {
                    ...
                    il.Emit(OpCodes.Callvirt, getItem); // stack is now [target][target][param-as-object]
                    ...
                    if (memberType == typeof(char) || memberType == typeof(char?))
                    {
                        il.EmitCall(OpCodes.Call, typeof(SqlMapper).GetMethod(
                            memberType == typeof(char) ? "ReadChar" : "ReadNullableChar", BindingFlags.Static | BindingFlags.Public), null); // stack is now [target][target][typed-param]
                    }
                    else
                    {
                        il.Emit(OpCodes.Dup); // stack is now [target][target][param][param]
                        il.Emit(OpCodes.Isinst, typeof(DBNull)); // stack is now [target][target][param-as-object][DBNull or null]
                        il.Emit(OpCodes.Brtrue_S, isDbNullLabel); // stack is now [target][target][param-as-object]
                        ...
                        if (unboxType.IsEnum)
                        {
                            ...
                            il.Emit(OpCodes.Dup); // stack is now [target][target][param][param]
                            il.Emit(OpCodes.Isinst, typeof(string)); // stack is now [target][target][param-as-object][string or null]
                            il.Emit(OpCodes.Dup);// stack is now [target][target][param-as-object][string or null][string or null]
                            StoreLocal(il, enumDeclareLocal); // stack is now [target][target][param-as-object][string or null]
                            il.Emit(OpCodes.Brfalse_S, isNotString); // stack is now [target][target][param-as-object]
                            ...
                            il.Emit(OpCodes.Unbox_Any, unboxType); // stack is now [target][target][typed-param]

                            if (nullUnderlyingType != null)
                            {
                                il.Emit(OpCodes.Newobj, memberType.GetConstructor(new[] { nullUnderlyingType })); // stack is now [target][target][enum-param]
                            }
                        }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                { // unbox as the data-type, then use IL-level convert
                                    il.Emit(OpCodes.Unbox_Any, dataType); // stack is now [target][target][data-typed-param]
                                    il.Emit(opCode); // stack is now [target][target][typed-param]
                                ...
                                { // use flexible conversion
                                    il.Emit(OpCodes.Ldtoken, unboxType); // stack is now [target][target][param][member-type-token]
                                    il.EmitCall(OpCodes.Call, typeof(Type).GetMethod("GetTypeFromHandle"), null); // stack is now [target][target][param][member-type]
                                    il.EmitCall(OpCodes.Call, typeof(Convert).GetMethod("ChangeType", new Type[] { typeof(object), typeof(Type) }), null); // stack is now [target][target][boxed-member-type-param]
                                    il.Emit(OpCodes.Unbox_Any, unboxType); // stack is now [target][target][typed-param]
                                }
                    ...
                    if (specializedConstructor == null)
                    {
                        // Store the param in the property/field
                    ...
                    il.MarkLabel(isDbNullLabel); // incoming stack: [target][target][param]
                    if (specializedConstructor != null)
                    {
                        il.Emit(OpCodes.Pop);
...
#pragma warning restore 612, 618
        ...
        /// <summary>
        /// Get the param of a parameter
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="name"></param>
        /// <returns>The param, note DBNull.Value is not returned, instead the param is returned as null</returns>
